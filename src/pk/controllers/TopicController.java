...
public class TopicController {
	...
	public String search(Model model,@Valid @ModelAttribute("topic") Topic topic,BindingResult br) throws Exception {	
		...
		if(readTopic == null) {
			model.addAttribute("err","No there is no current tutorials for that .<a style='color:white;' href='/solution/create'>Share a tutorial link </a> and help fellow geeks.");
		}
		else {
		List<Solution> solutions = searchService.searchByTopic(readTopic);
		if(solutions.isEmpty())
			model.addAttribute("err", "No there is no current tutorials for that .<a href='/solution/create'>Share a tutorial link </a> to help fellow geeks.");
